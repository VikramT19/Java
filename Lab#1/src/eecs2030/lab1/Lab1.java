...
public class Lab1 {
	...
	public static double windChill(double airTemp, double windSpeed) {
		return (13.12) + (0.6215 * airTemp) + ((0.3965*airTemp) - 11.37) * Math.pow(windSpeed, 0.16);
	}
	...
	public static boolean isOdd(int x) {
	    boolean result = (x%2 != 0);  // is remainder equal to zero?
	    return result;
	}
	...
	public static boolean isOnUnitCircle(double x, double y) {
		boolean result = (Math.pow(x, 2) + Math.pow(y, 2) == 1);
		return result;
	}
	...
	public static int flipBit(int x) {
		 if(x==0) {
			 ...
			 throw new BadBitException("Not valid");
		 }
		 return x;
	}
	...
	public static String toString(Range r) {
		String string = "range from " + r.getMinimum() + " to " + r.getMaximum();
		return string;
	}
	...
	public static String alternatingCaps(String s) {
        ...
        for (int i=0; i<s.length(); i++) {
            if (i%2 == 0) {
            	alternate += Character.toLowerCase(s.charAt(i));
            }
            else {
                alternate += Character.toUpperCase(s.charAt(i));
            }
        }
        return alternate; 
    }
