...
public class Firework {
	...
	public static Firework pearls(int n, Point2 position, double speed) {
		...
		for (int i = 0; i < n; i++) {
			...
			double x = (Firework.rng.nextDouble() - 0.5) * 10 + position.getX();
			double y = (Firework.rng.nextDouble() - 0.5) * 30 + position.getY();
		
			Point2 newPosition = new Point2(x,y);
		
			Particle p = new Particle(newPosition, vel, 5, Color.YELLOW);
	...
	public static Firework rings(Point2 position, double speed) {
		...
		for (int i = 0; i < 360; i += 10) {
			...
			double s = (Firework.rng.nextGaussian() * (0.03 * speed) + speed) * 0.5;
			vel.multiply(s);
			
			Particle p = new Particle(position, vel, 2, Color.red);
			f.particles.add(p);
		}
	...
	public static Firework fountain(int n, Point2 position, double speed) {
		...
		for (int i = 0; i < n; i++) {
			...
			double d = (Firework.rng.nextGaussian() * 2) + 90;
			Vector2 vel = Vector2.dirVector(d);
	...
	public static Firework pistil(Point2 position, double speed) {
		...
		for (int i = 0; i < 500; i++) {
			...
			double d = Firework.rng.nextDouble() * 360;
			Vector2 vel = Vector2.dirVector(d); 
			
			double s = Firework.rng.nextDouble() * (0.30 * speed);
			vel.multiply(s);
			
			Particle p = new Particle(position, vel, 2, Color.green);
			f.particles.add(p);
		}
