...
public class OwnedPiggyBank {
	...
	public void changeOwner(Owner currentOwner, Owner newOwner) {
		...
		if(!(this.owner.equals(currentOwner))) {
			throw new IllegalArgumentException("Current owner is not the current owner of this piggy bank.");
		}
	...
	public List<Coin> removeCoins(Owner user, int value) {
		...
		List<Coin> newBank = new ArrayList<Coin>();
		...
		for(int i = bank.size() - 1; i >= 0; i--) {
			...
			if(bank.get(i).getValue() <= value) {
				newBank.add(bank.get(i));
				value = value - bank.get(i).getValue();
			}
		}
		
		for (Coin coin : newBank) {
			bank.remove(coin);
		}
	...
	public List<Coin> deepCopy() {
		...
		List<Coin> copy = new ArrayList<>();
		
		for (int i = 0; i < bank.size(); i++) {
			copy.add(new Coin(this.bank.get(i)));
		}
